...
class VQA(Dataset):
    def __init__(self, image_set, root_path, data_path, ann_file, answer_vocab_file, use_imdb=True,
                 with_precomputed_visual_feat=True, boxes="36",
                 transform=None, test_mode=False,
                 zip_mode=False, cache_mode=False, cache_db=True, ignore_db_cache=True,
                 tokenizer=None, pretrained_model_name=None,
                 add_image_as_a_box=True, mask_size=(14, 14),
        ...
        # ========================== ianma ==========================
        infer_fn_train = "train_sample_1_num_5_top_k_0_top_p_0.9.json"
        infer_fn_val = "val_sample_1_num_5_top_k_0_top_p_0.9.json"
        with open('vqa/'+infer_fn_train, "r") as json_file:
            coco_high_infer_train = json.load(json_file)
        with open('vqa/'+infer_fn_val, "r") as json_file:
            coco_high_infer_val = json.load(json_file)
        coco_high_infer = coco_high_infer_train + coco_high_infer_val
        self.coco_high_infer_dict = {}
        top_k_gen = 1
        for entry in coco_high_infer:
            img_id = entry['img_fn'].split('/')[-1][:-len(".jpg")]
            if img_id in self.coco_high_infer_dict:
                if entry['event'] not in self.coco_high_infer_dict[img_id]['low_level_caps']:
                    self.coco_high_infer_dict[img_id]['low_level_caps'].append(entry['event'])

                if entry['inference_relation'] in self.coco_high_infer_dict[img_id]:
                    self.coco_high_infer_dict[img_id][entry['inference_relation']] += entry['generations'][:top_k_gen]
                else:
                    self.coco_high_infer_dict[img_id][entry['inference_relation']] = entry['generations'][:top_k_gen]

            else:
                self.coco_high_infer_dict[img_id] = {entry['inference_relation']: entry['generations'][:top_k_gen],
                                                     'low_level_caps'           : [entry['event'],]}

        # Main feature/notation loading
        self.image_feature_reader = ImageFeaturesH5Reader('data/coco/COCO.lmdb', dataset='coco')
        with open('data/coco/vqa/'+ann_file, 'r') as f:
            database_all = json.load(f)
        self.database = []
        for annot_i in database_all:
            raw_image_id = str(annot_i['image_id'])
            image_id = (12 - len(raw_image_id)) * '0' + raw_image_id
            if image_id in self.coco_high_infer_dict:
                self.database.append(annot_i)
        # ===========================================================
    ...
    def __getitem__(self, index):
        ...
        if self.with_precomputed_visual_feat:
            ...
            boxes_features = torch.as_tensor(np.array(boxes_features))
        ...
        # ========================== ianma ==========================
        # Add event / temporal / event + temporal
        low_level_cap = ""
        # print("low level cap #", len(self.coco_high_infer_dict[image_id]['low_level_caps']))
        for low_cap in self.coco_high_infer_dict[image_id]['low_level_caps']:
            if low_cap[-1] == ".":
                low_level_cap += " " + low_cap
            else:
                low_level_cap += " " + low_cap + " ."

        high_level_cap = " "
        for infer_rel in ["before", "after", "intent"]:
            # print("high level cap #", len(self.coco_high_infer_dict[image_id][infer_rel]))
            # high_level_cap += " . ".join(self.coco_high_infer_dict[image_id][infer_rel]) + ". "
            # high_level_cap += (" . " + infer_rel + " ").join(self.coco_high_infer_dict[image_id][infer_rel]) + ". "
            high_level_cap += infer_rel + " " + self.coco_high_infer_dict[image_id][infer_rel][0] + " . "

        q_tokens = self.tokenizer.tokenize(annot_i['question'] + high_level_cap)
        # print(annot_i['question'] + low_level_cap + high_level_cap)
        ...
        if not self.test_mode:
            answers = [ans_dict['answer'] for ans_dict in annot_i['answers']]
            if flipped:
