...
# ========================== ianma_begin ==========================
from oscar.image_feature_reader import ImageFeaturesH5Reader
import jsonlines
# ========================== ianma_end ==========================


# ========================== ianma_begin ==========================
class VCRCaptionGenDataset(Dataset):
    def __init__(self, split, tokenizer=None, add_od_labels=True,
        ...
        self.idx2id = []
        self.vcr_ann = {}
        with jsonlines.open("./datasets/vcr/{}.jsonl".format(split)) as reader:
            for ann in reader:
                img_id = self.extract_id(ann['img_id'])
                self.idx2id.append(img_id)
                self.vcr_ann[img_id] = ann

        self.image_feature_reader = ImageFeaturesH5Reader('../VL-BERT-IQET/data/vcr/VCR_resnet101_faster_rcnn_genome.lmdb', dataset='vcr')

    def extract_id(self, img_id):
        """
    ...
    def __getitem__(self, idx):
        img_key = self.idx2id[idx] # this can be vcr img_fn
        img_features = self.image_feature_reader[self.idx2id[idx]]['features']
        boxes = self.image_feature_reader[self.idx2id[idx]]['boxes']
        image_height = self.image_feature_reader[self.idx2id[idx]]['image_h']
        image_width = self.image_feature_reader[self.idx2id[idx]]['image_w']

        # https://github.com/microsoft/Oscar/issues/33
        print(image_height, image_width)
        print(boxes[0, :])
        # [141.1381      0.        478.88538   603.98834]
        # [767.33203   760.71735   898.1693    950.55914]
        ...
        spatial_features = np.concatenate(
            ...
            axis=1)
        full_features = np.concatenate((img_features, spatial_features), axis=1)

        features = torch.Tensor(full_features)
        caption = ""
        od_labels = None
...
def build_dataset(split, tokenizer, args, is_train=True):
    # ========================== ianma_begin ==========================
    ...
    return VCRCaptionGenDataset(split, tokenizer=tokenizer,
...
def test(args, test_dataset, model, tokenizer, predict_file):
    ...
    batch_num = int(len(test_dataset) / args.test_batch_size) + 1
    ...
    def gen_rows():
        ...
        with torch.no_grad():
            for step, (img_keys, batch) in tqdm(enumerate(test_dataloader), total=batch_num):
                is_exist = True
                for k in img_keys:
...
def main():
    ...
    parser.add_argument("--vcr_split", default='train', type=str, required=False,
                        help="VCR split")
    ...
    if args.do_test or args.do_eval:
        ...
        test_dataset = build_dataset(args.vcr_split,
                tokenizer, args, is_train=False)
        if args.n_gpu > 1:
            model = torch.nn.DataParallel(model)

        if not args.do_eval:
            predict_file = "vcr_generated_low_cap_{}.tsv".format(args.vcr_split)
