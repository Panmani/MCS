...
from dataloaders.vcr_generation import VCRGenDataset
...
def sample_sequence(model, length, context, end_token, pad_token, img_feats, boxes, boxes_mask, objects,
    ...
    if not do_sample:
        ...
        return beam_search_sequence(model, length, context, img_feats, boxes, boxes_mask, objects, segments,
                                    subject_ids, end_token, pad_token,
                                    num_beams=num_samples, temperature=1, device=device)
    if device is None:
        device = torch.device("cpu")
...
def main():
    ...
    parser.add_argument('--part_idx', type=int, default=0,
                       help="Part index")
    parser.add_argument("--split", default="all", type=str, choices=['all'],
    ...
    args.output_file = "{}_part{}_from_{}parts_".format(args.output_file, args.part_idx, args.part_num)
    args.cache_postfix = "{}_part{}".format(args.cache_postfix, args.part_idx)
    ...
    def _prompt_to_gen(context_orig, img_feats, boxes, boxes_mask, objects, segments, subject_ids):
    # ========================== ianma_end ==========================
    ...
    eval_dataset = dataset.get_dataset("all")
    ...
    for input_record, (
        ...
        if args.include_image:
            ...
            inputs, img_feats, boxes, boxes_mask, objects, segments, subject_ids = \
                [d.to(args.device) for d in data_input]
        else:
            ...
            img_feats, boxes, boxes_mask, objects, segments, subject_ids = [None] * 6
        ...
        generations = _prompt_to_gen(inputs, img_feats, boxes, boxes_mask, objects, segments, subject_ids)
