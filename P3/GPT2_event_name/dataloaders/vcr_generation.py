...
from utils.file_utils import read_and_parse_generation_json

from config import VCR_IMAGES_DIR, VCR_FEATURES_DIR, VCR_LOW_CAP_PATH
...
def _combine_and_pad_tokens(tokenizer: VisualCometTokenizer, tokens,
                            max_image, max_event, max_inference, max_seq_len):
    """
    ...
    max_lens = [max_image, max_event, max_inference]
    assert len(tokens) == len(max_lens)
    for i, t in enumerate(tokens):
        max_len = max_lens[i]
        if len(t) > max_len:
            # Truncate exceeding tokens
            if i < 2:
                if i == 0:
...
def vcg_record_to_tokens(tokenizer: VisualCometTokenizer,
    ...
    # ========================== ianma_begin ==========================
    event = record['event_name'] # event_name means event with persons' names added
    # place = record['place']
    # ========================== ianma_end ==========================
    ...
    def __init__(self,
        ...
        if os.path.exists(cached_features_files) and not overwrite_cache:
            ...
            with open(cached_features_files, 'rb') as handle:
                ...
                for split in splits:
                    examples[split], records[split] = p['data'][split]
        else:
            ...
            for s, split in enumerate(splits):
                ...
                # ========================== ianma_begin ==========================
                # split_filename = '{}_annots.json'.format(split)
                all_records = read_and_parse_generation_json(VCR_LOW_CAP_PATH)

                part_size = len(all_records) // self.part_num + 1
                print(part_size, len(all_records))
                start_at, end_at = self.part_idx * part_size, (self.part_idx + 1) * part_size
                if end_at < len(all_records):
                    print("Generate from {} to {}".format(start_at, end_at))
                    part_records = all_records[start_at: end_at]
                else:
                    print("Generate from {} to the end".format(start_at))
                    part_records = all_records[start_at:]

                records[split] = part_records
                # ========================== ianma_end ==========================
                ...
                for record in tqdm(records[split], "Encoding Data"):
                    ...
                    padded_tokens = _combine_and_pad_tokens(tokenizer, tokens, self.max_image, self.max_event, self.max_inference, self.max_seq_len)
                    ...
                    if not include_text: # mask out events and place text
                        ...
                        inference_end_token_idx = tokenizer.convert_tokens_to_ids([tokenizer.end_event])[0]
        ...
        for split in splits:
            self.vcg_dataset[split] = VCGGenLoader(examples[split], records[split], split, tokenizer,
...
class VCGGenLoader:
    def __init__(self, examples, records, split, tokenizer,
    ...
    def __getitem__(self, index):
        ...
        if not self.include_image:
            return event_inference_example
        ...
        # ========================== ianma_begin ==========================
        # return event_inference_example, features, boxes, boxes_mask, objects, segments, person_ids, subject_ids
        return event_inference_example, features, boxes, boxes_mask, objects, segments, subject_ids
        # ========================== ianma_end ==========================
